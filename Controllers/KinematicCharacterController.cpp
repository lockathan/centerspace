...
Notice from Jonathan Locke, 16th of August 2021

As per Point 2 above, the code below has been modified from the original source.
*/
...
KinematicCharacterController::KinematicCharacterController (btPairCachingGhostObject* ghostObject,btConvexShape* convexShape,btScalar stepHeight, int upAxis)
{
	...
	m_gravity = 9.8f ; // 3G acceleration.
	...
	Sequence = 0;
	m_ghostObject->setFriction(0.75f);
	//m_ghostObject->setRestitution(0.5f);

}
//---------------------------------------------------------------------------------------
KinematicCharacterController::~KinematicCharacterController ()
{
	PhysicsManager::getSingletonPtr()->removeCharacterController(this, m_ghostObject);
	if(m_ghostObject)
	{
...
void KinematicCharacterController::stepUp ( btCollisionWorld* world)
{
	...
	m_targetPosition = m_currentPosition + getUpAxisDirections()[m_upAxis] * (m_stepHeight + (m_verticalOffset));
	...
	btKinematicClosestNotMeConvexResultCallback callback (m_ghostObject, -getUpAxisDirections()[m_upAxis], btScalar(0.7071f));
...
void KinematicCharacterController::stepForwardAndStrafe ( btCollisionWorld* collisionWorld, const btVector3& walkMove)
{
   ...
   btTransform start, end;
   btVector3 old = btVector3(m_targetPosition);
...
void KinematicCharacterController::stepDown ( btCollisionWorld* collisionWorld, btScalar dt)
{
	...
	btVector3 old = btVector3(m_targetPosition);
	btVector3 old2 = btVector3(m_currentPosition);
   ...
   if (callback.hasHit())
   {
		...
		m_currentPosition.setInterpolate3 (m_currentPosition, m_targetPosition, callback.m_closestHitFraction);
		//m_currentPosition = btVector3(m_currentPosition.getX(), old2.y(), m_currentPosition.getZ());
...
void KinematicCharacterController::updateSequence(btScalar timeSinceLastFrame)
{
